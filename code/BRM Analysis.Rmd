...
Test model and figure

Resample 5 second intervals

```{r}
test_measures <- pupil_measures %>% 
  mutate(sec_5 = event_start_ms %/% 5000) %>% 
  group_by(participant, sec_5) %>% 
  summarise(pupil_diameter = mean(pupil_diameter, na.rm = TRUE))
```


```{r}
test_model <- lmer(data = test_measures,
                       pupil_diameter ~ bs(sec_5, df = 3, degree = 2) + (bs(sec_5, df = 3, degree = 2) | participant))
```
...
test_measures %>%
  ...
  geom_line(aes(y = predict(test_model), group = participant, col = "Subject"), size = 0.5) +
  geom_line(data = test_measures %>% distinct(sec_5), aes(y = predict(test_model, re.form = NA, newdata = test_measures %>% distinct(sec_5)), col="Population"), size = 1.5) +
...
Considering 1 to 9 knots placed at quantiles of time intervals.
...
for (i in 2:10){
  ...
  PS_model <- lmer(data = fixation_intervals,
                       pupil_mean ~ bs(event_interval, df = i, degree = 1) + (bs(event_interval, df = i, degree = 1) | ID))

  AIC_s[i-1] <- AICc(PS_model)
  BIC_s[i-1] <- BIC(PS_model)
}

ggplot(data = data_frame(knots = 1:9, AIC = AIC_s, BIC = BIC_s)) +
